# .NET 8 minimal API starter

Minimal .NET 8 minimal API starter for Orbit Cloud. Deploys with zero configuration: connect the repo
...
`TargetFramework` in the .csproj, or a global.json. Generic pins in `.tool-versions` or `mise.toml` also work. Declare the
