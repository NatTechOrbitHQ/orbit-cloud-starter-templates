...
Minimal Ruby (Sinatra) starter for Orbit Cloud. Deploys with zero configuration: connect the repo
...
the `ruby` directive in the Gemfile, or a `.ruby-version` file. Generic pins in `.tool-versions` or `mise.toml` also work. Declare the
