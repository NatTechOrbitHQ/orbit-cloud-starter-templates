...
Minimal Java (Maven) starter for Orbit Cloud. Deploys with zero configuration: connect the repo
...
`maven.compiler.source`/`target` in pom.xml, or a `.tool-versions` entry. Generic pins in `.tool-versions` or `mise.toml` also work. Declare the
