# Elixir (Plug/Cowboy) starter

Minimal Elixir (Plug/Cowboy) starter for Orbit Cloud. Deploys with zero configuration: connect the repo
...
the `elixir` requirement in mix.exs. Generic pins in `.tool-versions` or `mise.toml` also work. Declare the
