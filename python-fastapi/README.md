...
Minimal Python (FastAPI) starter for Orbit Cloud. Deploys with zero configuration: connect the repo
...
a `.python-version` file, or `requires-python` in pyproject.toml. Generic pins in `.tool-versions` or `mise.toml` also work. Declare the
